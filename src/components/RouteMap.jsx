  ...
  return (
    <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:16, overflow:'hidden', marginBottom:16, boxShadow:'0 2px 8px rgba(36,56,0,0.06)' }}>
      <div style={{ padding:'14px 20px', borderBottom:`1px solid ${BORDER}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <p style={{ fontWeight:700, fontSize:15, color:MOSS, textTransform:'uppercase', letterSpacing:'0.06em' }}>Route Map</p>
        {pickup && dropoff && <span style={{ fontSize:15, color:MOSS, fontWeight:600 }}>{pickup} → {dropoff}</span>}
      </div>
      ...
      {pickup && dropoff ? (
        <div style={{ position:'relative' }}>
          {(!token || token === 'your_mapbox_public_token_here' || !pCoord || !dCoord) ? (
            <div style={{ height:280, background:OLIVE, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:10, position:'relative', overflow:'hidden' }}>
              ...
              <p style={{ color:'rgba(255,255,255,0.7)', fontSize:15, textAlign:'center', position:'relative' }}>
                {!pCoord || !dCoord
                  ? <>Route preview unavailable for this pickup/dropoff.</>
                  : <>Add <code style={{ color:NEON, fontSize:15 }}>VITE_MAPBOX_TOKEN</code> to <code style={{ color:NEON, fontSize:15 }}>.env</code><br/>to see the route map</>
                }
              </p>
            </div>
          ) : (
          ...
          <div style={{ padding:'14px 20px', borderTop:`1px solid ${BORDER}`, display:'flex', flexDirection:'column', gap:10 }}>
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              ...
              <div>
                <p style={{ fontSize:14.5, color:MUTED, fontWeight:500 }}>Pickup</p>
                <p style={{ fontSize:15.5, color:TEXT, fontWeight:600 }}>{pickup}</p>
              </div>
            </div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              ...
              <div>
                <p style={{ fontSize:14.5, color:MUTED, fontWeight:500 }}>Drop-off</p>
                <p style={{ fontSize:15.5, color:TEXT, fontWeight:600 }}>{dropoff}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ height:200, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:10, background:BG }}>
          ...
          <p style={{ fontSize:15, color:MUTED, textAlign:'center' }}>Select pickup and drop-off<br/>to see your route on the map</p>
        </div>
      )}
    </div>
  )
