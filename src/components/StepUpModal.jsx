  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1200, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        {sending ? (
          <p style={{ fontSize: 15.5, color: MUTED, padding: '18px 0', textAlign: 'center' }}>Sending a code to your email…</p>
        ) : sendError ? (
          <div style={{ textAlign: 'center', padding: '10px 0' }}>
            <p style={{ fontSize: 15.5, color: '#ef4444', marginBottom: 14 }}>{sendError}</p>
            <button onClick={requestCode} style={{ padding: '10px 22px', borderRadius: 50, background: NEON, color:ON_NEON, fontWeight: 700, fontSize: 15.5, border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>Try again</button>
          </div>
        ) : (
          <form onSubmit={submit}>
            <p style={{ fontSize: 15.5, color: MUTED, lineHeight: 1.6, margin: '2px 0 16px' }}>
            ...
            <button type="submit" disabled={busy || code.length !== 6}
              style={{ width: '100%', marginTop: 16, padding: '13px', borderRadius: 50, background: (busy || code.length !== 6) ? BORDER : NEON, color: (busy || code.length !== 6) ? MUTED : ON_NEON, fontWeight: 800, fontSize: 16, border: 'none', cursor: (busy || code.length !== 6) ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
              {busy ? 'Verifying…' : 'Confirm'}
            </button>
            <button type="button" onClick={requestCode} disabled={busy}
              style={{ width: '100%', marginTop: 8, padding: '10px', borderRadius: 50, background: 'transparent', color: MUTED, fontWeight: 600, fontSize: 15, border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
              Resend code
            </button>
          </form>
        )}
