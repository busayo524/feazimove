    ...
    <div style={{ position: 'relative' }} ref={dropRef}>
      <div style={{
        ...
        >
          ...
          <span style={{ fontSize: 15, fontWeight: 700, color: '#555', letterSpacing: '0.04em' }}>{country.code}</span>
          {!disabled && (
        ...
        {/* Dial code */}
        <span style={{ fontSize: 15, fontWeight: 700, color: '#444', padding: '0 8px 0 10px', flexShrink: 0, userSelect: 'none' }}>
          {country.dial}
        </span>
        ...
        <input
          ...
          style={{
            flex: 1, padding: '13px 10px 13px 10px', fontSize: 16,
        ...
        {localNum && !disabled && (
          <button type="button" onClick={() => { setLocalNum(''); onChange('') }}
            style={{ padding: '0 12px', background: 'none', border: 'none', cursor: 'pointer', color: '#bbb', fontSize: 16, lineHeight: 1, flexShrink: 0 }}
        ...
        <div style={{
          ...
          <div style={{ padding: '8px 10px', borderBottom: '1px solid #f0f0f0' }}>
            <input
              ...
              style={{
                width: '100%', padding: '7px 10px', fontSize: 15, borderRadius: 7,
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {filtered.map(c => (
              ...
              >
                ...
                <span style={{ fontSize: 15, color: '#333', flex: 1 }}>{c.name}</span>
                <span style={{ fontSize: 15, color: '#999', fontWeight: 600 }}>{c.dial}</span>
              </button>
            ))}
            {filtered.length === 0 && (
              <p style={{ textAlign: 'center', color: '#aaa', fontSize: 15, padding: 16, margin: 0 }}>No results</p>
            )}
          </div>
        </div>
      )}
