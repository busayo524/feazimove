  ...
  if (tokenMissing) {
    return (
      <div style={{ background:OLIVE, borderRadius:20, height:340, marginBottom:20, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:8 }}>
        ...
        <p style={{ color:'rgba(255,255,255,0.7)', fontSize:15, textAlign:'center', padding:'0 20px' }}>
  ...
  if (sdkError) {
    return (
      <div style={{ background:OLIVE, borderRadius:20, height:340, marginBottom:20, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:10 }}>
        <MapPin size={26} color={NEON}/>
        <p style={{ color:'rgba(255,255,255,0.7)', fontSize:15, textAlign:'center', padding:'0 20px' }}>Couldn't load the map.</p>
        <button onClick={() => setRetryKey(k => k + 1)}
          style={{ padding:'7px 18px', borderRadius:50, background:NEON, color:ON_NEON, fontWeight:700, fontSize:15, border:'none', cursor:'pointer', fontFamily:'inherit' }}>
          Retry
        </button>
      </div>
    )
  ...
  if (!pCoord || !dCoord) {
    return (
      <div style={{ background:CARD, border:`1.5px dashed ${BORDER}`, borderRadius:20, padding:24, marginBottom:20, textAlign:'center' }}>
        ...
        <p style={{ fontSize:15, color:MUTED }}>Route preview unavailable for this pickup/dropoff.</p>
      </div>
    )
  }
