...
export default function Login() {
  ...
  // One-time notice when the user was auto-signed-out for inactivity
  const [idleNotice, setIdleNotice] = useState(() => {
    if (!sessionStorage.getItem(IDLE_LOGOUT_FLAG)) return ''
    sessionStorage.removeItem(IDLE_LOGOUT_FLAG)
    return 'You were signed out after 45 minutes of inactivity. Please sign in again.'
  })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f2f3f4', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
      ...
      {/* Logo — side by side. Links home in the browser, inert in the installed app. */}
      ...
      <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 24px rgba(0,0,0,0.08)', padding: 'clamp(36px,5vw,52px)', width: '100%', maxWidth: 520 }}>
        ...
        <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          {/* Idle session notice */}
          {idleNotice && !error && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 10, padding: '11px 14px' }} role="status">
              <Clock size={15} style={{ color: '#d97706', flexShrink: 0 }} />
              <p style={{ color: '#92400e', fontSize: 13, margin: 0 }}>{idleNotice}</p>
            </div>
          )}
