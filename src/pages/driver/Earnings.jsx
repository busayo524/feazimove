  ...
  return(
    <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, padding:'16px 18px', flex:1, minWidth:130, boxShadow:'0 2px 8px rgba(36,56,0,0.05)' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
        <p style={{ fontSize:13.5, color:MUTED, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em' }}>{label}</p>
      ...
      {sub && <p style={{ fontSize:13.5, color:MUTED, marginTop:4 }}>{sub}</p>}
    </div>
  )
}
      ...
      {data.map((d, i) => {
        ...
        return(
          ...
          <div key={i} style={{ flex:'1 1 0', minWidth:0, display:'flex', flexDirection:'column', alignItems:'center', gap:5 }}>
            ...
            <span style={{ fontSize:11.5, fontWeight: isHighest ? 700 : 500, color:isHighest ? ON_NEON : MUTED,
  ...
  return(
    <AppLayout title="Earnings">
      ...
      <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:20, padding:20, marginBottom:16, display:'flex', alignItems:'center', gap:16, boxShadow:'0 2px 8px rgba(36,56,0,0.06)' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:4 }}>
            ...
            <span style={{ color:MOSS, fontSize:14, fontWeight:600 }}>Online · FeaziMove Driver</span>
          </div>
          <p style={{ color:MUTED, fontSize:13.5, marginTop:2 }}>{user?.phone || '—'}</p>
        </div>
        <div style={{ textAlign:'right' }}>
          <p style={{ color:MUTED, fontSize:12.5, fontWeight:600 }}>WALLET</p>
        ...
        {PROFILE_STATS.map(s=>(
          <div key={s.label} style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, padding:'14px 16px', boxShadow:'0 2px 8px rgba(36,56,0,0.05)', display:'flex', alignItems:'center', gap:12, minWidth:0, overflow:'hidden' }}>
            ...
            <div style={{ minWidth:0 }}>
              <p style={{ fontSize:12.5, color:MUTED, fontWeight:600 }}>{s.label}</p>
      ...
      <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:16, padding:20, marginBottom:16, display:'flex', alignItems:'center', gap:16, boxShadow:'0 2px 8px rgba(36,56,0,0.06)' }}>
        <div style={{ flex:1 }}>
          <p style={{ color:MOSS, fontSize:13.5, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:4 }}>Available to Withdraw</p>
          <p style={{ color:OLIVE, fontWeight:900, fontSize:26, letterSpacing:'-0.03em' }}>{fmt(walletBalance)}</p>
          <p style={{ color:MUTED, fontSize:13.5, marginTop:4 }}>Processed within 24 hours</p>
        </div>
        <button onClick={() => setShowWithdraw(true)} disabled={walletBalance <= 0}
          style={{ padding:'13px 20px', borderRadius:12, background:walletBalance>0?OLIVE:BORDER, color:walletBalance>0?NEON:MUTED, fontWeight:800, fontSize:14.5, border:'none', cursor:walletBalance>0?'pointer':'not-allowed', display:'flex', alignItems:'center', gap:8, boxShadow:'0 2px 8px rgba(36,56,0,0.2)', fontFamily:'inherit' }}>
      ...
      {showWithdraw && (
        <div style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,0.4)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background:CARD, borderRadius:16, padding:24, maxWidth:340, width:'100%', boxShadow:'0 12px 32px rgba(0,0,0,0.2)' }}>
            ...
            <p style={{ fontSize:14, color:MUTED, marginBottom:4 }}>Available: {fmt(walletBalance)}. Requests are reviewed by an admin before payout.</p>
            <p style={{ fontSize:13.5, color:MUTED, marginBottom:14 }}>
            ...
            <form onSubmit={handleWithdraw}>
              ...
              {withdrawError && <p style={{ fontSize:14, color:'#ef4444', marginBottom:12 }}>{withdrawError}</p>}
              {withdrawSuccess && <p style={{ fontSize:14, color:'#15803d', marginBottom:12 }}>{withdrawSuccess}</p>}
              ...
              <div style={{ display:'flex', gap:10 }}>
                ...
                <button type="submit" disabled={withdrawing}
                  style={{ flex:1, padding:'11px', borderRadius:10, border:'none', background:NEON, color:ON_NEON, fontWeight:700, fontSize:14.5, cursor:withdrawing?'not-allowed':'pointer', fontFamily:'inherit', opacity:withdrawing?0.7:1 }}>
      ...
      <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:16, overflow:'hidden', marginBottom:16, boxShadow:'0 2px 8px rgba(36,56,0,0.06)' }}>
        <div style={{ padding:'14px 20px', borderBottom:`1px solid ${BORDER}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <p style={{ fontWeight:700, fontSize:14, color:MOSS, textTransform:'uppercase', letterSpacing:'0.06em' }}>Recent Transactions</p>
          <span style={{ fontSize:13.5, color:MUTED }}>{txns.length} records</span>
        </div>

        {txns.length === 0 ? (
          <div style={{ padding:'28px 20px', textAlign:'center', color:MUTED, fontSize:14.5 }}>
          ...
          txns.slice(0,20).map((t,i,arr)=>(
            <div key={t.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'13px 20px', borderBottom:i<arr.length-1?`1px solid ${BORDER}`:'none', transition:'background 0.15s' }}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <p style={{ color:TEXT, fontWeight:600, fontSize:14.5, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{t.description}</p>
                <p style={{ color:MUTED, fontSize:13.5, marginTop:2 }}>{t.date}</p>
              </div>
              <span style={{ fontWeight:800, fontSize:14, color:t.type==='credit'? ON_NEON :'#ef4444', background: t.type==='credit'?NEON:DANGER_SOFT, padding:'3px 11px', borderRadius:20, flexShrink:0 }}>
      ...
      <div style={{ display:'flex', gap:6, marginBottom:16, background:CARD, border:`1px solid ${BORDER}`, borderRadius:12, padding:5, boxShadow:'0 2px 8px rgba(36,56,0,0.06)' }}>
        {[['today','Today'],['week','This Week'],['month','This Month']].map(([val,label])=>(
          <button key={val} onClick={()=>setPeriod(val)}
            style={{ flex:1, padding:'9px', borderRadius:9, fontSize:14, fontWeight:700, border:'none', background:period===val?NEON:CARD, color:period===val? ON_NEON :MUTED, cursor:'pointer', transition:'all 0.2s', fontFamily:'inherit' }}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:20, padding:'24px', marginBottom:16, position:'relative', overflow:'hidden', boxShadow:'0 2px 8px rgba(36,56,0,0.06)' }}>
        ...
        <p style={{ color:MUTED, fontSize:13.5, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:4 }}>{periodLabel}</p>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,minmax(0,1fr))', gap:12 }}>
          {[
            ...
            <div key={l} style={{ background:'rgba(36,56,0,0.08)', borderRadius:10, padding:'10px 12px', minWidth:0, overflow:'hidden' }}>
              <p style={{ color:MUTED, fontSize:12.5, marginBottom:3 }}>{l}</p>
              <p style={{ color:OLIVE, fontWeight:800, fontSize:14.5, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{v}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:16, padding:'18px 20px', marginBottom:16, boxShadow:'0 2px 8px rgba(36,56,0,0.06)' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline',
          ...
          <p style={{ fontWeight:700, fontSize:14, color:MOSS, textTransform:'uppercase', letterSpacing:'0.06em' }}>
            {period==='month' ? 'Monthly Breakdown' : 'Weekly Breakdown'}
          </p>
          <span style={{ fontSize:14, color:MUTED, fontWeight:600, whiteSpace:'nowrap' }}>
      ...
      <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:16, overflow:'hidden', boxShadow:'0 2px 8px rgba(36,56,0,0.06)' }}>
        <div style={{ padding:'14px 20px', borderBottom:`1px solid ${BORDER}` }}>
          <p style={{ fontWeight:700, fontSize:14, color:MOSS, textTransform:'uppercase', letterSpacing:'0.06em' }}>Earnings Summary</p>
        </div>
        {[
          ...
          <div key={row.label} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'13px 20px', borderBottom:i<arr.length-1?`1px solid ${BORDER}`:'none' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              <div>
                <p style={{ fontSize:14.5, fontWeight:600, color:TEXT }}>{row.label}</p>
                <p style={{ fontSize:13.5, color:MUTED, marginTop:1 }}>{row.sub}</p>
              </div>
            </div>
