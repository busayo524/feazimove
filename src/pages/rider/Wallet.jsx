  ...
  return (
    <div id="funding-account-card" style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 20, marginBottom: 20, boxShadow: '0 2px 8px rgba(36,56,0,0.06)' }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
        ...
        <p style={{ fontWeight: 700, fontSize: 14, color: MOSS, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Your Personal Funding Account</p>
      </div>
      ...
      {account && kyc && !kyc.isNamed && kyc.canReverify && !showForm && (
        <div style={{ background: kyc.verifying ? NEON_SOFT : WARN_SOFT,
          ...
          <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:6 }}>
            ...
            <p style={{ fontSize:14, fontWeight:800, color: kyc.verifying ? MOSS : '#9a3412' }}>
              {kyc.verifying ? 'Verifying your identity…' : 'This account is not in your name yet'}
            </p>
          </div>
          <p style={{ fontSize:13.5, color:MUTED, lineHeight:1.5, marginBottom: kyc.verifying ? 0 : 10 }}>
            {kyc.verifying
          ...
          {!kyc.verifying && (
            <button onClick={() => setShowForm(true)}
              style={{ padding:'9px 16px', borderRadius:10, background:'none', border:`1.5px solid ${OLIVE}`, color:OLIVE, fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:'inherit' }}>
      ...
      {!showForm && account ? (
        <>
          {[['Bank', account.bankName], ['Account Number', account.accountNumber, true], ['Account Name', account.accountName]].map(([label, value, copy]) => (
            <div key={label} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 0', borderBottom:'1px solid #f2f4ee' }}>
              ...
              <span style={{ display:'flex', alignItems:'center', gap:2, fontSize:14.5, fontWeight:700, color:TEXT }}>
                {value || '—'}{copy && value && <CopyBtn text={String(value)}/>}
              </span>
            </div>
          ))}
          <p style={{ fontSize:13.5, color:MUTED, marginTop:10 }}>
            Transfer any amount to this account any time — it lands in your FeaziMove wallet automatically.
          </p>
        </>
      ) : !showForm && pending ? (
        <p style={{ fontSize:14, color:MOSS, display:'flex', alignItems:'center', gap:8 }}>
        ...
        <form onSubmit={submit}>
          <p style={{ fontSize:14, color:MUTED, marginBottom:12, lineHeight:1.5 }}>
            {account
          ...
          <input value={bvn} onChange={e => setBvn(e.target.value.replace(/[^0-9]/g, '').slice(0, 11))} placeholder="BVN (11 digits)" inputMode="numeric" required
            style={{ width:'100%', padding:'11px 14px', borderRadius:10, fontSize:14.5, border:`1.5px solid ${BORDER}`, marginBottom:10, boxSizing:'border-box', background:CARD, color:TEXT, fontFamily:'inherit' }}/>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:10, marginBottom:10 }}>
            <div style={{ minWidth:0 }}>
              <label htmlFor="wallet-dob" style={{ display:'block', fontSize:13.5, fontWeight:600, color:MUTED, marginBottom:5 }}>
                Date of Birth
              </label>
              <input id="wallet-dob" type="date" value={dob} onChange={e => setDob(e.target.value)} required max={today}
                style={{ width:'100%', minWidth:0, maxWidth:'100%', padding:'11px 12px', borderRadius:10, fontSize:14.5, border:`1.5px solid ${BORDER}`, boxSizing:'border-box', background:CARD, color:TEXT, fontFamily:'inherit', WebkitAppearance:'none', appearance:'none' }}/>
            </div>
            <div style={{ minWidth:0 }}>
              <label htmlFor="wallet-gender" style={{ display:'block', fontSize:13.5, fontWeight:600, color:MUTED, marginBottom:5 }}>
                Gender
              </label>
              <select id="wallet-gender" value={gender} onChange={e => setGender(e.target.value)} required
                style={{ width:'100%', minWidth:0, maxWidth:'100%', padding:'11px 12px', borderRadius:10, fontSize:14.5, border:`1.5px solid ${BORDER}`, boxSizing:'border-box', background:CARD, color:TEXT, fontFamily:'inherit' }}>
          ...
          {error && <p style={{ fontSize:14, color:'#ef4444', marginBottom:10 }}>{error}</p>}
          {!busy && missing.length > 0 && (
            <p style={{ fontSize:13.5, color:MUTED, marginBottom:10, lineHeight:1.5 }}>
              Still needed: {missing.join(', ')}.
            </p>
          )}
          <div style={{ display:'flex', gap:10 }}>
            <button type="submit" disabled={!canSubmit}
              style={{ flex:1, padding:'11px', borderRadius:10, background:!canSubmit?BORDER:NEON, color:!canSubmit?MUTED:ON_NEON, border:'none', fontWeight:800, fontSize:14.5, cursor:!canSubmit?'not-allowed':'pointer', fontFamily:'inherit' }}>
              {busy ? (account ? 'Verifying…' : 'Creating…') : (account ? 'Try Again' : 'Create My Account')}
            </button>
            <button type="button" onClick={() => setShowForm(false)}
              style={{ padding:'11px 16px', borderRadius:10, background:'none', border:`1.5px solid ${BORDER}`, color:MUTED, fontWeight:700, fontSize:14.5, cursor:'pointer', fontFamily:'inherit' }}>
              Cancel
            </button>
          </div>
        </form>
        ...
        <>
          ...
          <button onClick={() => setShowForm(true)}
            style={{ padding:'10px 18px', borderRadius:10, background:'none', border:`1.5px solid ${OLIVE}`, color:OLIVE, fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:'inherit' }}>
            Set Up My Account
          </button>
        </>
      )}
  ...
  return (
    <AppLayout title="Wallet">
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 20, padding: '28px 24px', marginBottom: 20, position: 'relative', overflow: 'hidden', boxShadow: '0 2px 8px rgba(36,56,0,0.06)' }}>
        ...
        <p style={{ color:MUTED, fontSize: 14, fontWeight: 600, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Available Balance</p>
        <p style={{ color: OLIVE, fontWeight: 900, fontSize: 'clamp(2rem,5vw,3rem)', letterSpacing: '-0.03em', lineHeight: 1 }}>
          {balance === null ? '—' : `₦${balance.toLocaleString()}`}
        </p>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10 }}>
          <p style={{ color:MUTED, fontSize: 14 }}>{user?.phone || '••• ••• ••••'}</p>
          {pendingFund && <span style={{ fontSize: 13.5, color: MOSS, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
            <RefreshCw size={11} style={{ animation: 'spin 1s linear infinite' }} /> Waiting for transfer…
          </span>}
        </div>
        {successMsg && (
          <div style={{ marginTop: 12, background: NEON, color:ON_NEON, fontWeight: 700, fontSize: 14, padding: '8px 14px', borderRadius: 8, display: 'inline-block' }}>
            ✓ {successMsg}
          </div>
        )}
        {bvnSetUp === true && (
          <button onClick={() => { setWithdrawError(''); setShowWithdraw(true) }}
            style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 8, padding: '10px 18px', borderRadius: 10,
              background:ACCENT_FILL, color:ON_ACCENT_FILL, border: 'none', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 20, marginBottom: 20, boxShadow: '0 2px 8px rgba(36,56,0,0.06)' }}>
        <p style={{ fontWeight: 700, fontSize: 14, color: MOSS, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 14 }}>Top Up Wallet</p>
        {!pendingFund && (
          <>
            <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
              {QUICK.map(q => (
                <button key={q} onClick={() => setAmount(String(q))}
                  style={{ padding: '8px 16px', borderRadius: 50, fontSize: 14, fontWeight: 700, border: `1.5px solid ${amount === String(q) ? NEON : BORDER}`, background: amount === String(q) ? NEON : BG, color: amount === String(q) ? ON_NEON : MOSS, cursor: 'pointer', transition: 'all 0.15s' }}>
                  ₦{q.toLocaleString()}
                </button>
              ))}
            </div>
            <form onSubmit={handleFund} style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              ...
              <button type="submit" disabled={funding}
                style={{ padding: '12px 20px', borderRadius: 10, background: funding ? BORDER : NEON, color: funding ? MUTED : ON_NEON, border: 'none', fontWeight: 800, fontSize: 14.5, cursor: funding ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {bvnSetUp === false && !pendingFund && (
          <button onClick={goToSetup}
            ...
            <span style={{ fontSize:14, color:TEXT, lineHeight:1.45 }}>
        ...
        {fundError && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 10, background: DANGER_SOFT, border: '1px solid #fca5a5', borderRadius: 8, padding: '8px 12px' }}>
            ...
            <span style={{ color: '#ef4444', fontSize: 14 }}>{fundError}</span>
          </div>
        )}
      </div>
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 8px rgba(36,56,0,0.06)' }}>
        <div style={{ padding: '14px 20px', borderBottom: `1px solid ${BORDER}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <p style={{ fontWeight: 700, fontSize: 14, color: MOSS, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Transaction History</p>
          <span style={{ fontSize: 13.5, color: MUTED }}>{transactions.length} records</span>
        </div>

        {loadingData ? (
          <div style={{ padding: '32px 20px', textAlign: 'center', color: MUTED, fontSize: 14.5 }}>Loading…</div>
        ) : dataError ? (
          <div style={{ padding: '32px 20px', textAlign: 'center', color: '#ef4444', fontSize: 14.5 }}>{dataError}</div>
        ) : transactions.length === 0 ? (
          <div style={{ padding: '32px 20px', textAlign: 'center', color: MUTED, fontSize: 14.5 }}>No transactions yet.</div>
          ...
          return (
            <div key={txn.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 20px', borderBottom: i < transactions.length - 1 ? `1px solid ${BORDER}` : 'none', transition: 'background 0.15s' }}
              ...
              <div style={{ flex: 1 }}>
                <p style={{ color: TEXT, fontWeight: 600, fontSize: 14.5 }}>{txn.description}</p>
                <span style={{ color: MUTED, fontSize: 13.5 }}>{txn.date}</span>
              </div>
              <span style={{ fontWeight: 800, fontSize: 14.5, flexShrink: 0, background: isCredit ? NEON : DANGER_SOFT, color:isCredit ? ON_NEON : '#ef4444', padding: '3px 12px', borderRadius: 20 }}>
      ...
      {showWithdraw && (
        <div style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,0.4)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background:CARD, borderRadius:16, padding:24, maxWidth:340, width:'100%', boxShadow:'0 12px 32px rgba(0,0,0,0.2)' }}>
            ...
            <p style={{ fontSize:13.5, color:MUTED, marginBottom:14 }}>A 5% processing fee applies to withdrawals.</p>
            <form onSubmit={startWithdraw}>
              <label style={{ display:'block', fontSize:14, fontWeight:600, color:TEXT, marginBottom:6 }}>Amount (₦)</label>
              <input type="number" min="100" max={balance ?? 0} value={withdrawAmount}
                ...
                style={{ width:'100%', padding:'10px 12px', borderRadius:10, border:`1.5px solid ${BORDER}`, fontSize:14.5, marginBottom:6, fontFamily:'inherit', boxSizing:'border-box', background:CARD, color:TEXT, caretColor:TEXT }}/>
              {wAmount >= 100 && (
                <p style={{ fontSize:14, color:MOSS, fontWeight:700, marginBottom:10 }}>
                  You'll receive ₦{(wNetKobo / 100).toLocaleString(undefined, { maximumFractionDigits: 2 })} after the 5% fee.
                </p>
              )}
              {withdrawError && <p style={{ fontSize:14, color:'#ef4444', marginBottom:10 }}>{withdrawError}</p>}
              {withdrawSuccess && <p style={{ fontSize:14, color:'#15803d', marginBottom:10 }}>{withdrawSuccess}</p>}
              <div style={{ display:'flex', gap:10 }}>
                ...
                <button type="submit"
                  style={{ flex:1, padding:'11px', borderRadius:10, border:'none', background:NEON, color:ON_NEON, fontWeight:700, fontSize:14.5, cursor:'pointer', fontFamily:'inherit' }}>
                  Withdraw
                </button>
              </div>
            </form>
