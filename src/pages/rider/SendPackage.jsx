    ...
    return(
      <AppLayout title="Send Package">
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:380,textAlign:'center',padding:24}}>
          ...
          <button onClick={()=>setSent(false)} style={{padding:'13px 32px',borderRadius:50,background:NT,color:NEON,fontWeight:700,fontSize:15,border:'none',cursor:'pointer'}}>
            Send Another
          </button>
        </div>
      </AppLayout>
    ...
    <AppLayout title="Send Package">
      <form onSubmit={handleSubmit}>
        <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:16,padding:20,marginBottom:16,boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
          <p style={{fontWeight:700,fontSize:14,color:TEXT,marginBottom:14}}>Package Size</p>
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            {SIZES.map(s=>(
              <label key={s.id} style={{display:'flex',alignItems:'center',gap:14,padding:'14px 16px',borderRadius:12,border:`2px solid ${size===s.id?NEON:BORDER}`,background:size===s.id?NT:CARD,cursor:'pointer',transition:'all 0.15s'}}>
                ...
                <p style={{fontSize:13,fontWeight:700,color:size===s.id?NEON:TEXT}}>{s.est}</p>
              </label>
            ))}
          </div>
        </div>
        ...
        <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:16,padding:20,marginBottom:16,boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
          <p style={{fontWeight:700,fontSize:14,color:TEXT,marginBottom:14}}>Route</p>
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            {[['Pickup Location','pickup','e.g. 12 Allen Avenue, Ikeja'],['Drop-off Location','dropoff','e.g. 5 Admiralty Way, Lekki']].map(([label,key,ph])=>(
              <div key={key}>
                ...
                <input value={form[key]} onChange={e=>set(key,e.target.value)} placeholder={ph} style={INPUT} onFocus={e=>e.target.style.borderColor=NEON} onBlur={e=>e.target.style.borderColor=BORDER} required/>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:16,padding:20,marginBottom:20,boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
          <p style={{fontWeight:700,fontSize:14,color:TEXT,marginBottom:14}}>Recipient Details</p>
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            {[['Recipient Name','recipient','text','Full name'],['Recipient Phone','recipientPhone','tel','+234 800 000 0000']].map(([label,key,type,ph])=>(
              <div key={key}>
                <label style={{display:'block',fontSize:13,fontWeight:600,color:TEXT,marginBottom:6}}>{label}</label>
                <input type={type} value={form[key]} onChange={e=>set(key,e.target.value)} placeholder={ph} style={INPUT} onFocus={e=>e.target.style.borderColor=NEON} onBlur={e=>e.target.style.borderColor=BORDER} required/>
              </div>
            ))}
            <div>
              <label style={{display:'block',fontSize:13,fontWeight:600,color:TEXT,marginBottom:6}}>Notes (optional)</label>
              <textarea value={form.notes} onChange={e=>set('notes',e.target.value)} placeholder="Fragile, handle with care..." rows={3} style={{...INPUT,resize:'vertical'}} onFocus={e=>e.target.style.borderColor=NEON} onBlur={e=>e.target.style.borderColor=BORDER}/>
            </div>
          </div>
        </div>

        <button type="submit" disabled={submitting} style={{width:'100%',padding:'15px',borderRadius:50,background:submitting?'#ccc':NT,color:submitting?'#fff':NEON,fontWeight:700,fontSize:15,border:'none',cursor:submitting?'not-allowed':'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:10}}>
