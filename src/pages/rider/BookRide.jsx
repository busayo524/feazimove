    ...
    <div style={{ display:'flex', alignItems:'center', gap:10, background:NEON_SOFT, border:`1.5px solid ${NEON}`,
      ...
      <div style={{ minWidth:0, flex:1 }}>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <p style={{ fontSize:15.5, fontWeight:800, color:TEXT, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{d.name}</p>
          <span style={{ display:'flex', alignItems:'center', gap:3, fontSize:15, fontWeight:700, color:OLIVE, flexShrink:0 }}>
            <Star size={11} fill="#eab308" color="#eab308"/>{(d.rating ?? 5).toFixed(1)}
          </span>
        </div>
        {(d.vehicleMake || d.vehicleModel || d.plateNumber) && (
          <p style={{ display:'flex', alignItems:'center', gap:5, fontSize:14.5, color:MUTED, marginTop:2 }}>
            <Car size={11}/>
        ...
        <div style={{padding:'10px 14px'}}>
          ...
          <div style={{display:'flex',gap:8}}>
            ...
            <div style={{flex:1,background:CARD,border:`1.5px solid ${BORDER}`,borderRadius:10,padding:'8px 10px',textAlign:'center'}}>
              {fareKobo!=null
                ? <span style={{fontSize:15.5,fontWeight:800,color:OLIVE}}>{`₦${Math.round(fareKobo/100).toLocaleString()}`}</span>
                : <span style={{fontSize:14.5,fontWeight:600,color:MUTED}}>Set once matched</span>}
            </div>
          </div>
        ...
        <div style={{padding:'0 14px 12px'}}>
          {matching ? (
            <>
              {candidateDrivers?.length > 0 && (
                <div style={{ marginBottom:4 }}>
                  <p style={{ fontSize:15, fontWeight:800, color:OLIVE, textTransform:'uppercase', letterSpacing:'0.05em', margin:'6px 0 8px' }}>
                  ...
                  <p style={{ fontSize:14.5, color:MUTED, lineHeight:1.4, marginBottom:4 }}>
            ...
            <>
              ...
              <p style={{fontSize:14.5,color:MUTED,textAlign:'center',margin:'8px 0 0',lineHeight:1.4}}>
        ...
        <>
          ...
          {payPending && !showPreview && (
            <button onClick={()=>setShowPreview(true)}
              style={{ position:'fixed', bottom:18, left:'50%', transform:'translateX(-50%)', zIndex:900,
                ...
                background:OLIVE, color:NEON, border:'none', fontWeight:800, fontSize:15,
          ...
          <div className="bookride-scroll">
            ...
            <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:12,marginBottom:10,boxShadow:'0 2px 8px rgba(36,56,0,0.06)'}}>
              <p style={{fontWeight:700,fontSize:15,color:MOSS,textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:8}}>What do you need?</p>
                ...
                {SERVICES.map(s=>{
                  ...
                  return(
                    <button key={s.id} onClick={()=>{ setService(s.id); setScheduledOk(null) }} style={{
                      ...
                      <p style={{fontWeight:700,fontSize:14.5,color:active? ON_NEON :TEXT,marginBottom:1,lineHeight:1.15}}>{s.label}</p>
            ...
            <div>
              ...
              <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:12,marginBottom:10,boxShadow:'0 2px 8px rgba(36,56,0,0.06)'}}>
                <div style={{display:'grid',gridTemplateColumns:'minmax(min(104px,100%),0.38fr) minmax(0,1fr)',gap:8}}>
                  <TimeDropdown
                    slots={MERIDIEMS}
                    value={half}
                    onChange={chooseHalf}
              ...
              <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:12,marginBottom:10,boxShadow:'0 2px 8px rgba(36,56,0,0.06)'}}>
                <div style={{display:'flex',flexDirection:'column',gap:10}}>
                  ...
                  />
                  <div>
                    <label style={{display:'block',fontSize:15,fontWeight:600,color:TEXT,marginBottom:6}}>Additional Comment (optional)</label>
                    <textarea value={comment} onChange={e=>setComment(e.target.value.slice(0,200))}
                      ...
                      style={{width:'100%',padding:'12px 14px',borderRadius:10,fontSize:15.5,border:`1.5px solid ${BORDER}`,outline:'none',background:CARD,color:TEXT,fontFamily:'inherit',resize:'vertical',boxSizing:'border-box'}}
              ...
              {upcoming.length > 0 && (
                <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:12,marginBottom:10,boxShadow:'0 2px 8px rgba(36,56,0,0.06)'}}>
                  <p style={{fontWeight:700,fontSize:15,color:MOSS,textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:8}}>
                  ...
                  {upcoming.map(s=>(
                    <div key={s.bookingId} style={{border:`1.5px solid ${s.isToday?NEON:BORDER}`,borderRadius:12,
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                        ...
                        {s.fare!=null && (
                          <span style={{marginLeft:'auto',fontSize:15,fontWeight:800,color:OLIVE,
                      ...
                      {s.driver ? (
                        <div style={{display:'flex',alignItems:'center',gap:8,marginTop:8}}>
                          ...
                          <div style={{minWidth:0,flex:1}}>
                            ...
                            <p style={{fontSize:14,color:MUTED,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
