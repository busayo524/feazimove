...
const steps = [
  { num: '01', icon: 'down',  title: 'Create an account & sign in', desc: 'Sign up on FeaziMove in minutes. Provide your details, complete verification, and you are ready to ride or send.', detail: 'A few documents are required for verification — this keeps the community safe and trusted.', img: accessAccount, imgAlt: 'Create an account' },
...
export default function HowItWorksPage() {
  return (
    <LandingLayout>
      ...
      {/* HERO */}
      <section style={{ background: 'var(--bg)', padding: '120px 20px 0' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center', marginBottom: 40 }}>
          <p style={{ fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-faint)', marginBottom: 32 }}>Simple Process</p>
          <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(2.4rem,6vw,4rem)', letterSpacing: '-0.04em', color: 'var(--text)', lineHeight: 1.1, margin: '0 0 20px' }}>
            Four steps to your first ride
          </h1>
          <p style={{ fontSize: 'clamp(1rem,2vw,1.15rem)', color: 'var(--text-muted)', lineHeight: 1.75, maxWidth: 580, margin: '0 auto' }}>
            ...
            return (
              <div key={num} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '80px', alignItems: 'center', padding: '80px 0', borderBottom: i < steps.length - 1 ? '1px solid var(--border)' : 'none' }}>
                ...
                <div style={{ order: flip ? 1 : 2 }}>
                  ...
                  <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(2rem,3.5vw,3rem)', letterSpacing: '-0.04em', color: 'var(--text)', lineHeight: 1.1, marginBottom: 16, display: 'block' }}>{title}</h3>
                  <p style={{ fontSize: '1rem', color: 'var(--text-muted)', lineHeight: 1.8, marginBottom: 12 }}>{desc}</p>
                  <p style={{ fontSize: '0.875rem', color: 'var(--text-faint)', fontWeight: 400 }}>{detail}</p>
                </div>
              </div>
            )
          })}
      ...
      {/* FOR DRIVERS */}
      <div style={{ background: 'var(--bg)', padding: '0 clamp(16px,4vw,60px) 80px' }}>
        <section style={{ background: 'var(--bg-raised)', borderRadius: 24, padding: '80px clamp(24px,5vw,80px)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '60px', alignItems: 'center', position: 'relative', zIndex: 1 }}>
            <div>
              <p style={{ fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text-faint)', marginBottom: 16 }}>For Drivers</p>
              <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(2.2rem,4vw,3.4rem)', letterSpacing: '-0.04em', color: 'var(--text)', lineHeight: 1.0, margin: '0 0 24px' }}>
                Drive on your<br />own schedule
              </h2>
              <p style={{ fontSize: '1rem', color: 'var(--text-muted)', lineHeight: 1.8, marginBottom: 28 }}>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 36px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                {['Flexible hours — drive when you want', 'Earn more per trip with pooling', 'Real-time earnings dashboard', 'Fast weekly payouts to your bank'].map(item => (
                  <li key={item} style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: '0.95rem', color: 'var(--text-muted)' }}>
                    <span style={{ width: 22, height: 22, borderRadius: '50%', background: 'rgba(204,255,0,0.35)', color: 'var(--text)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, flexShrink: 0 }}>&#10003;</span>
                    {item}
                  </li>
                ))}
              </ul>
      ...
      {/* CTA */}
      <section style={{ padding: '80px 20px', textAlign: 'center', background: 'var(--bg)', borderTop: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(1.8rem,3.5vw,2.6rem)', letterSpacing: '-0.04em', color: 'var(--text)', marginBottom: 16 }}>
