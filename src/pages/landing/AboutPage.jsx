    ...
    <LandingLayout>
      ...
      <section style={{ background: 'var(--bg)', padding: '100px clamp(20px,6vw,80px)' }}>
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          <p style={{ fontSize: '0.72rem', fontWeight: 800, letterSpacing: '0.18em', textTransform: 'uppercase', color: C.textFaint, textAlign: 'center', marginBottom: 20 }}>Our Story</p>
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(2.4rem,5vw,3.8rem)', letterSpacing: '-0.04em', color: C.text, lineHeight: 1.05, textAlign: 'center', marginBottom: 56 }}>
            How FeaziMove began
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
            <p style={{ fontSize: '1.1rem', color: C.textMid, lineHeight: 1.85 }}>
      ...
      {/* ── Why FeaziMove ───────────────────────────────────────────────── */}
      <section style={{ background: C.subtleBg, padding: '100px clamp(20px,6vw,80px)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 24 }}>
            {[
              ...
              <div key={num} style={{ background: C.cardBg, borderRadius: 20, padding: '36px 32px', position: 'relative', overflow: 'hidden', border: isDark ? '1px solid rgba(255,255,255,0.07)' : 'none' }}>
                <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: '5rem', color: C.numColor, lineHeight: 1, position: 'absolute', top: 12, right: 20, letterSpacing: '-0.04em', userSelect: 'none' }}>{num}</div>
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#ccff00', marginBottom: 24 }} />
                <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: '1.25rem', color: C.text, marginBottom: 14, lineHeight: 1.2 }}>{title}</h3>
                <p style={{ fontSize: '0.95rem', color: C.textSoft, lineHeight: 1.8 }}>{body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-28" style={{ background: 'var(--bg-card)', borderTop: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 clamp(20px,5vw,60px)', display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 32 }}>
          ...
          {/* The Feazi Way */}
          <div className="card p-10 md:p-14" style={{ display: 'flex', flexDirection: 'column', gap: 20, background: C.feaziWayBg }}>
            <p style={{ fontSize: '0.7rem', fontWeight: 800, letterSpacing: '0.18em', color: C.textFaint, textTransform: 'uppercase', marginBottom: 8 }}>The Feazi Way</p>
            <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(1.8rem,3.2vw,2.6rem)', lineHeight: 1.05, color: C.text, margin: 0, whiteSpace: 'nowrap' }}>How we do it</h3>
            <p style={{ fontSize: '1.08rem', color: C.textSoft, lineHeight: 1.85, margin: 0 }}>Pooled transport along shared routes — connecting commuters and goods moving the same way, making every journey more feasible, easy, and affordable. That&apos;s the Feazi Way.</p>
          </div>
        </div>
      </section>
