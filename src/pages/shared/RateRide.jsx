    ...
    return(
      <AppLayout title="Rate Ride">
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:400,textAlign:'center',padding:24}}>
          ...
          <p style={{color:MUTED,fontSize:16,marginBottom:10}}>{submitted ? `You rated your ${otherLabel}` : `You already rated this ${otherLabel}`}</p>
          {submitted && (
          ...
          <button onClick={()=>navigate(homeRoute())} style={{padding:'13px 32px',borderRadius:50,background:NEON,color:ON_NEON_HARD,fontWeight:700,fontSize:16,border:'none',cursor:'pointer'}}>
            Done
          </button>
        </div>
      </AppLayout>
    ...
    <AppLayout title="Rate Ride">
      ...
      <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:16,padding:20,marginBottom:16,boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
        <div style={{display:'flex',alignItems:'center',gap:14}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <p style={{fontWeight:700,fontSize:16,color:TEXT}}>{other?.name || `Your ${otherLabel}`}</p>
            <p style={{fontSize:15,color:MUTED,marginTop:2}}>{ride.pickup} → {ride.destination}</p>
            <p style={{fontSize:15,color:MUTED,marginTop:1}}>{ride.date}</p>
          </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:16,padding:24,marginBottom:16,boxShadow:'0 1px 3px rgba(0,0,0,0.04)',textAlign:'center'}}>
          <p style={{fontWeight:700,fontSize:16,color:TEXT,marginBottom:6}}>How was your {isRider ? 'ride' : 'rider'}?</p>
          <p style={{fontSize:15,color:MUTED,marginBottom:20}}>Tap a star to rate</p>
          ...
          {stars>0&&<p style={{fontWeight:700,fontSize:16,color:ON_NEON_HARD,background:NEON,display:'inline-block',padding:'4px 16px',borderRadius:20}}>{['','Poor','Fair','Good','Great','Excellent!'][stars]}</p>}
        </div>
        ...
        {submitError && (
          <div style={{display:'flex',alignItems:'flex-start',gap:10,padding:'12px 14px',background:DANGER_SOFT,border:'1px solid #fca5a5',borderRadius:12,marginBottom:16}}>
            ...
            <p style={{fontSize:15,color:'#ef4444'}}>{submitError}</p>
          </div>
        )}

        <button type="submit" disabled={!stars||submitting} style={{width:'100%',padding:'15px',borderRadius:50,background:!stars||submitting?'#e5e7eb':NEON,color:!stars||submitting?MUTED:ON_NEON_HARD,fontWeight:700,fontSize:16,border:'none',cursor:!stars||submitting?'not-allowed':'pointer',transition:'background 0.2s'}}>
