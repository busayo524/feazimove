  ...
  return (
    <AdminLayout title="User Management">
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        <p style={{ margin:0, color:MUTED, fontSize:15.5 }}>{users.length} user{users.length !== 1 ? 's' : ''} registered</p>
        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
          <button onClick={handleExport} disabled={exporting}
            style={{ display:'flex', alignItems:'center', gap:8, padding:'10px 18px', borderRadius:10, background:CARD,
              border:`1px solid ${BORDER}`, color:TEXT, fontWeight:700, fontSize:16, cursor: exporting?'wait':'pointer', fontFamily:'inherit' }}>
        ...
        <div style={{ position:'relative', flex:'1 1 200px', minWidth:180 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            style={{ width:'100%', padding:'9px 12px 9px 34px', borderRadius:8, border:`1.5px solid ${BORDER}`,
              fontSize:15.5, fontFamily:'inherit', background:CARD, color:TEXT, boxSizing:'border-box', outline:'none' }}/>
        </div>
        ...
        <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
          {FILTERS.map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{ padding:'7px 16px', borderRadius:50, fontSize:16, fontWeight:600, cursor:'pointer', border:'none',
        ...
        <div style={{ position:'relative' }}>
          <button onClick={() => setRoleOpen(o => !o)}
            style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 14px', borderRadius:8,
              border:`1.5px solid ${BORDER}`, background:CARD, color:TEXT, fontWeight:600, fontSize:16,
              ...
              {ROLES.map(r => (
                <button key={r} onClick={() => { setRole(r); setRoleOpen(false) }}
                  style={{ display:'block', width:'100%', padding:'10px 16px', textAlign:'left',
                    ...
                    fontWeight: role === r ? 700 : 400, fontSize:15.5, border:'none',
            ...
            <thead>
              <tr style={{ background:CARD, textAlign:'left' }}>
                {['User','Role','Joined','Status',''].map(h => (
                  <th key={h} style={{ padding:'12px 16px', fontSize:15, color:MUTED, fontWeight:700,
              ...
              {loading ? (
                ...
                return (
                  <tr key={u.id} style={{ borderTop:`1px solid ${BORDER}` }}>
                    ...
                    <td style={{ padding:'12px 16px' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                        ...
                        <div>
                          ...
                          <p style={{ fontSize:15.5, color:MUTED, margin:0 }}>{u.email || u.phone}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Role */}
                    <td style={{ padding:'12px 16px' }}>
                      <span style={{ fontSize:15, fontWeight:700, padding:'3px 10px', borderRadius:20, background:badge.bg, color:badge.fg,
                        border:`1px solid ${badge.bd || badge.fg + '22'}`, textTransform:'capitalize' }}>
                        {u.role}
                      </span>
                    </td>
                    {/* Joined */}
                    <td style={{ padding:'12px 16px', color:MUTED, fontSize:16, whiteSpace:'nowrap' }}>
                      {new Date(u.joinedAt).toLocaleDateString()}
                    </td>

                    {/* Status */}
                    <td style={{ padding:'12px 16px' }}>
                      <span style={{ fontSize:15, fontWeight:700, padding:'4px 12px', borderRadius:20,
...
function AddUserPanel({ onClose, onCreated }) {
  ...
  const label = { display:'block', fontSize:16, fontWeight:700, color:TEXT, marginBottom:8 }
  const field = { width:'100%', padding:'10px 12px', borderRadius:8, border:`1.5px solid ${BORDER}`,
    fontSize:15.5, fontFamily:'inherit', boxSizing:'border-box', background:CARD, color:TEXT, outline:'none' }
    ...
    <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:12, padding:'18px 20px',
      ...
      {created ? (
        <div style={{ display:'flex', alignItems:'center', gap:20, flexWrap:'wrap' }}>
          ...
          <button onClick={onClose}
            style={{ padding:'10px 24px', borderRadius:8, background:NEON, color:ON_NEON, border:'none',
              fontWeight:700, fontSize:15.5, cursor:'pointer', fontFamily:'inherit', flexShrink:0 }}>
            Done
          </button>
        </div>
      ) : (
        <form onSubmit={handleSubmit}>
          <div style={{ display:'flex', gap:16, flexWrap:'wrap', alignItems:'flex-end' }}>
            ...
            <button type="submit" disabled={busy}
              style={{ padding:'10px 22px', borderRadius:8, background:NEON, color:ON_NEON, border:'none', fontWeight:700,
                fontSize:15.5, cursor:busy?'not-allowed':'pointer', fontFamily:'inherit', opacity:busy?0.7:1, flexShrink:0, whiteSpace:'nowrap' }}>
          ...
          <p style={{ fontSize:15.5, color:MUTED, margin:'12px 0 0', lineHeight:1.5 }}>
          ...
          {error && <p style={{ fontSize:16, color:'#ef4444', margin:'10px 0 0' }}>{error}</p>}
        </form>
      )}
    </div>
  )
