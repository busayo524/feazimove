    ...
    <div style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,0.4)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
      ...
      <div onClick={e => e.stopPropagation()} style={{ background:CARD, borderRadius:16, padding:24, maxWidth:380, width:'100%', boxShadow:'0 12px 32px rgba(0,0,0,0.2)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
          <p style={{ fontWeight:800, fontSize:16, color:TEXT, margin:0 }}>{created ? 'Admin User Created' : 'Add Admin User'}</p>
        ...
        {created ? (
          <div>
            ...
            <div style={{ background:BG, borderRadius:10, padding:14, marginBottom:16 }}>
              <p style={{ fontSize:12, color:MUTED, margin:'0 0 4px' }}>Email</p>
              <p style={{ fontWeight:700, color:TEXT, margin:'0 0 10px' }}>{created.user.email}</p>
              <p style={{ fontSize:12, color:MUTED, margin:'0 0 4px' }}>Temporary Password</p>
              <p style={{ fontWeight:700, color:TEXT, fontFamily:'monospace', margin:0 }}>{created.temporaryPassword}</p>
            </div>
