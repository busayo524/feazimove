  ...
  return (
    <AdminLayout title="KYC Access Log">
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:12, flexWrap:'wrap', marginBottom:20 }}>
        <p style={{ color:MUTED, fontSize:15.5, maxWidth:640, lineHeight:1.6 }}>
        ...
        <button onClick={handleExport} disabled={exporting}
          style={{ display:'flex', alignItems:'center', gap:7, padding:'9px 16px', borderRadius:10, border:'none',
            background:NEON, color:ON_NEON, fontWeight:800, fontSize:15.5, cursor: exporting?'wait':'pointer', fontFamily:'inherit' }}>
      ...
      {!entries ? (
        ...
        <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, padding:40, textAlign:'center' }}>
          ...
          <p style={{ color:MUTED, fontSize:16 }}>No KYC access recorded yet.</p>
        </div>
      ) : (
              ...
              <thead>
                <tr style={{ background:BG, textAlign:'left' }}>
                  {['When', 'Event', 'Admin', 'Whose record', 'From'].map(h => (
                    <th key={h} style={{ padding:'10px 14px', fontSize:15, color:MUTED, textTransform:'uppercase',
                    ...
                    <tr key={e.id} style={{ borderTop:'1px solid #f5f5f5' }}>
                      ...
                      <td style={{ padding:'11px 14px', whiteSpace:'nowrap' }}>
                        <span style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'3px 10px',
                          borderRadius:50, background:ic.bg, color:ic.fg, fontWeight:700, fontSize:15 }}>
                      ...
                      <td style={{ padding:'11px 14px' }}>
                        ...
                        {e.actorEmail && e.actorEmail !== e.actorName && (
                          <p style={{ fontSize:15, color:MUTED }}>{e.actorEmail}</p>
                        )}
                      ...
                      <td style={{ padding:'11px 14px', color:MUTED, fontSize:15.5 }}>
                        ...
                        {e.userAgent && (
                          <p style={{ fontSize:15, maxWidth:260, overflow:'hidden', textOverflow:'ellipsis',
