import { ADMIN_CARD as CARD, ADMIN_BORDER as BORDER, ADMIN_TEXT as TEXT, ADMIN_MUTED as MUTED, NEON, DANGER_SOFT } from '../../theme/palette'
...
function BarChart({ data }) {
  if (!data.length) return <p style={{ color:MUTED, fontSize:14.5 }}>No revenue recorded in the last 14 days.</p>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={() => handleExport('rides')} disabled={exporting==='rides'}
            style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', borderRadius:10, border:`1px solid ${BORDER}`, background:CARD, color:TEXT, fontWeight:600, fontSize:13.5, cursor:'pointer', fontFamily:'inherit' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1.4fr 1fr', gap:16 }} className="admin-reports-grid">
            <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, overflow:'hidden' }}>
              ...
              {data.topRoutes.length === 0 ? (
                <p style={{ color:MUTED, fontSize:14.5, padding:'0 18px 18px' }}>No rides yet.</p>
                ...
                <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 18px', borderTop:`1px solid ${BORDER}` }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    <span style={{ fontSize:13.5, color:MUTED }}>{r.count} trip{r.count!==1?'s':''}</span>
                    <span style={{ fontSize:13.5, fontWeight:700, color:'#3f6212', background:'#f3fbd3', padding:'2px 8px', borderRadius:20 }}>{r.sharePct}%</span>
                  </div>
                </div>
              ))}
            </div>
