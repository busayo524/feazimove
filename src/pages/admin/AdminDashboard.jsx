...
function CardHeader({ title, sub, icon, viewAll }) {
  return (
    <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:14 }}>
      <div>
        <p style={{ fontSize:16, fontWeight:700, color:TEXT, margin:0 }}>{title}</p>
        {sub && <p style={{ fontSize:15.5, color:MUTED, margin:'3px 0 0' }}>{sub}</p>}
      </div>
      {viewAll ? (
        <Link to={viewAll} style={{ display:'flex', alignItems:'center', gap:5, fontSize:16, fontWeight:600,
  ...
  return (
    <Card style={{ padding:18 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
        <p style={{ fontSize:16, color:MUTED, fontWeight:600, margin:0 }}>{label}</p>
        <div style={{ width:32, height:32, borderRadius:9, background:'#f3fbd3', display:'flex',
      ...
      {sub && <p style={{ fontSize:15.5, color:MUTED, marginTop:5, marginBottom:0 }}>{sub}</p>}
    </Card>
  )
}
...
function Tooltip({ tip }) {
  ...
  return (
    <div style={{ position:'absolute', left:tip.x, top:tip.y, transform:'translate(-50%, -110%)',
      background:'#1a1a1a', color:'#fff', borderRadius:8, padding:'7px 10px', fontSize:15.5,
...
function StatusDonut({ data }) {
  ...
  return (
    <div ref={boxRef} style={{ position:'relative', display:'flex', flexDirection:'column', alignItems:'center', gap:14 }}>
      {total === 0 ? (
        <p style={{ fontSize:16, color:MUTED, padding:'60px 0' }}>No rides yet</p>
      ) : (
      ...
      <div style={{ display:'flex', flexWrap:'wrap', justifyContent:'center', gap:'6px 16px' }}>
        {data.map(d => (
          <div key={d.status} style={{ display:'flex', alignItems:'center', gap:6 }}>
            ...
            <span style={{ fontSize:15.5, color:MUTED }}>{STATUS_LABELS[d.status]}</span>
            <span style={{ fontSize:15.5, fontWeight:700, color:TEXT }}>{d.count}</span>
          </div>
        ))}
      </div>
      <Tooltip tip={tip}/>
...
function RouteBars({ data }) {
  ...
  if (data.length === 0) return <p style={{ fontSize:16, color:MUTED, padding:'40px 0', textAlign:'center' }}>No rides yet</p>
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14, paddingTop:4 }}>
      {data.map(d => (
        <div key={d.route}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5, gap:10 }}>
            <span style={{ fontSize:15.5, color:TEXT, fontWeight:600, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{d.route}</span>
            <span style={{ fontSize:15.5, color:MUTED, fontWeight:600, flexShrink:0, fontVariantNumeric:'tabular-nums' }}>{d.count}</span>
          </div>
...
export default function AdminDashboard() {
  ...
  const two = { display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(min(320px,100%),1fr))', gap:16, marginBottom:16 }

  return (
    <AdminLayout title="Dashboard">
      <p style={{ color:MUTED, fontSize:15.5, marginBottom:20 }}>Ride analytics and activity overview.</p>
      ...
      {error && (
        <div style={{ display:'flex', gap:8, padding:'10px 14px', background:DANGER_SOFT, border:'1px solid #fca5a5', borderRadius:10, marginBottom:20 }}>
          ...
          <p style={{ fontSize:16, color:'#ef4444', margin:0 }}>{error}</p>
        </div>
      )}
      ...
      {!data ? (
        ...
        <>
          {/* ── Overview stat cards ─────────────────────────────────────── */}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(min(210px,100%),1fr))', gap:16, marginBottom:16 }} className="dash-stat-grid">
              ...
              {alertRows(alerts).slice(0, 6).map((a, i) => (
                <div key={a.key} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 0',
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <p style={{ fontSize:16, fontWeight:700, color:TEXT, margin:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                      {a.title}
                    </p>
                    <p style={{ fontSize:15.5, color:MUTED, margin:'3px 0 0', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{a.sub}</p>
                  </div>
                  <span style={{ fontSize:15, fontWeight:700, padding:'3px 10px', borderRadius:50,
                    background:a.bg, color:a.fg, border:`1px solid ${a.bd}`, flexShrink:0 }}>{a.tag}</span>
                </div>
              ))}
              {alertRows(alerts).length > 6 && (
                <p style={{ fontSize:15.5, color:MUTED, margin:'10px 0 0' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'minmax(0, 2fr) minmax(min(280px,100%), 1fr)', gap:16, marginBottom:16 }}
          ...
          <div style={two}>
            <Card>
              ...
              {data.recentRides.length === 0 ? (
                <p style={{ fontSize:16, color:MUTED, padding:'30px 0', textAlign:'center' }}>No rides yet</p>
                ...
                return (
                  <div key={r.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 0',
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <p style={{ fontSize:15.5, color:MUTED, margin:'3px 0 0' }}>
            ...
            <Card>
              ...
              {data.topDrivers.length === 0 ? (
                <p style={{ fontSize:16, color:MUTED, padding:'30px 0', textAlign:'center' }}>No drivers yet</p>
                ...
                <div key={d.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 0',
                  ...
                  <div style={{ width:36, height:36, borderRadius:'50%', background:'#ccff00', color:'#243800',
                    display:'flex', alignItems:'center', justifyContent:'center', fontWeight:800, fontSize:15.5, flexShrink:0 }}>
                    {(d.name || '?').charAt(0).toUpperCase()}
                  </div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <p style={{ fontSize:16, fontWeight:700, color:TEXT, margin:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{d.name}</p>
                    <p style={{ fontSize:15.5, color:MUTED, margin:'3px 0 0' }}>
                      {d.vehicle || 'No vehicle on file'} · {d.trips} trip{d.trips === 1 ? '' : 's'}
                    </p>
                  </div>
                  <span style={{ fontSize:15, fontWeight:700, padding:'3px 10px', borderRadius:50, flexShrink:0,
          ...
          <Card>
            ...
            {data.recentActivity.length === 0 ? (
              <p style={{ fontSize:16, color:MUTED, padding:'30px 0', textAlign:'center' }}>No activity yet</p>
            ) : data.recentActivity.map((a, i) => (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:14, padding:'13px 0',
                borderTop: i > 0 ? `1px solid ${BORDER}` : 'none' }}>
                <Clock size={16} color={MUTED} style={{ flexShrink:0 }}/>
                <div style={{ flex:1, minWidth:0 }}>
                  <p style={{ fontSize:16, fontWeight:700, color:TEXT, margin:0 }}>{a.action}</p>
                  <p style={{ fontSize:15.5, color:MUTED, margin:'3px 0 0', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                    {a.actor}{a.detail ? ` · ${a.detail}` : ''} · {new Date(a.at).toLocaleString('en-NG', { timeZone:'Africa/Lagos' })}
                  </p>
                </div>
                <span style={{ fontSize:15, fontWeight:600, padding:'3px 10px', borderRadius:50,
