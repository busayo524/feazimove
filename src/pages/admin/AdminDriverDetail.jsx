      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        ...
        <button onClick={toggleStatus} disabled={busy}
          style={{ display:'flex', alignItems:'center', gap:8, padding:'9px 16px', borderRadius:10, border:'1px solid #fca5a5',
            background:'#fff', color:'#ef4444', fontWeight:700, fontSize:16, cursor:busy?'not-allowed':'pointer', fontFamily:'inherit' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:16 }} className="admin-detail-grid">
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, padding:18 }}>
            <p style={{ fontWeight:800, fontSize:16, color:TEXT, marginBottom:14 }}>Driver Information</p>
            <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1fr) minmax(0,1fr)', gap:14, fontSize:16 }}>
          ...
          {/* Vehicle */}
          <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, padding:18 }}>
            <p style={{ fontWeight:800, fontSize:16, color:TEXT, marginBottom:14 }}>Vehicle</p>
            <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1fr) minmax(0,1fr)', gap:14, fontSize:16 }}>
        ...
        <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, padding:18, alignSelf:'start' }}>
          <p style={{ fontWeight:800, fontSize:16, color:TEXT, marginBottom:14 }}>Documents</p>
          {driver.documents.length === 0 ? (
            <p style={{ color:MUTED, fontSize:16 }}>No documents uploaded.</p>
            ...
            <div key={d.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 0', borderBottom:'1px solid #f5f5f5' }}>
              <span style={{ display:'flex', alignItems:'center', gap:8, fontSize:16, color:TEXT }}>
                <FileText size={14} color={MUTED}/> {DOC_LABELS[d.type] || d.type}
              </span>
              <button onClick={() => viewDocument(d.id)} style={{ background:'none', border:'none', color:'#2563eb', fontWeight:600, fontSize:15.5, cursor:'pointer', fontFamily:'inherit' }}>
                View
              </button>
            </div>
          ))}
  ...
  return (
    <div style={{ marginTop:14, background:t.bg, border:`1px solid ${t.bd}`, borderRadius:10, padding:'11px 14px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:7 }}>
        ...
        <p style={{ margin:0, fontWeight:800, fontSize:15.5, color:t.fg, letterSpacing:'0.03em' }}>{t.label}</p>
      </div>
      {identity.summary && (
        <p style={{ margin:'6px 0 0', fontSize:15.5, color:t.fg, lineHeight:1.5 }}>{identity.summary}</p>
      )}
      {identity.licenceName && (
        <p style={{ margin:'6px 0 0', fontSize:15.5, color:MUTED }}>
...
function Info({ label, value }) {
  return (
    <div style={{ minWidth:0 }}>
      <p style={{ color:MUTED, fontSize:15, marginBottom:2 }}>{label}</p>
