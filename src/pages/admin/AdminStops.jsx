...
export default function AdminStops() {
  ...
  return (
    <AdminLayout title="Stops">
      ...
      <p style={{ color:MUTED, fontSize:13.5, marginBottom:20 }}>
            ...
            <div key={side} style={{ display:'flex', flexDirection:'column', gap:14 }}>
              ...
              {zones.filter(z => z.side === side).map(zone => {
                ...
                return (
                  <div key={zone.id}
                    ...
                    {list.length === 0 && (
                      <p style={{ padding:'14px 18px', fontSize:13.5, color:MUTED }}>No stops yet — drag one here.</p>
                    )}
                      ...
                      <div key={s.id} draggable={!busy}
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:4, minWidth:0 }}>
                          ...
                          <div style={{ minWidth:0 }}>
                            <p style={{ fontSize:14.5, fontWeight:600, color:TEXT, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{s.name}</p>
                            <p style={{ fontSize:12.5, color:MUTED }}>#{i + 1}{!s.isActive && ' · Inactive'}</p>
                          </div>
                        </div>
                        <div style={{ display:'flex', alignItems:'center', gap:2, flexShrink:0, position:'relative' }}>
                          ...
                          {moveMenuFor === s.id && (
                            <div style={{ position:'absolute', right:0, top:'100%', zIndex:20, background:CARD, border:`1px solid ${BORDER}`,
                              ...
                              <p style={{ fontSize:12.5, fontWeight:700, color:MUTED, padding:'8px 12px 4px' }}>Move to…</p>
                              {zones.filter(z => z.side === side && z.id !== zone.id).map(z => (
                                <button key={z.id} onClick={() => moveToZone(s, z)}
                                  style={{ display:'block', width:'100%', textAlign:'left', padding:'8px 12px', background:'none', border:'none',
                                    cursor:'pointer', fontSize:14.5, color:TEXT, fontFamily:'inherit' }}>
                                  {z.name}
                                </button>
                              ))}
                              {zones.filter(z => z.side === side && z.id !== zone.id).length === 0 && (
                                <p style={{ fontSize:13.5, color:MUTED, padding:'8px 12px' }}>No other category yet.</p>
                              )}
                            </div>
                          )}
                          <button onClick={() => toggleActive(s)}
                            style={{ display:'flex', alignItems:'center', gap:5, background:'none', border:'none', cursor:'pointer',
                              color: s.isActive ? '#ef4444' : '#15803d', fontWeight:600, fontSize:12.5, fontFamily:'inherit' }}>
                ...
                <div style={{ background:CARD, border:`1.5px dashed #f59e0b`, borderRadius:14, overflow:'hidden' }}>
                  ...
                  {orphans(side).map(s => (
                    <div key={s.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:8, padding:'10px 14px 10px 18px', borderBottom:'1px solid #f5f5f5', position:'relative' }}>
                      <p style={{ fontSize:14.5, fontWeight:600, color:TEXT }}>{s.name}{!s.isActive && <span style={{ color:MUTED, fontWeight:400 }}> · Inactive</span>}</p>
                      <button onClick={() => setMoveMenuFor(moveMenuFor === s.id ? null : s.id)}
                        style={{ display:'flex', alignItems:'center', gap:5, background:'none', border:'none', cursor:'pointer', color:OLIVE, fontWeight:600, fontSize:12.5, fontFamily:'inherit' }}>
              ...
              <button onClick={() => addZone(side)}
                style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6, padding:'11px', borderRadius:12,
                  background:'none', border:`1.5px dashed ${BORDER}`, color:MUTED, fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:'inherit' }}>
    ...
    <div style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,0.4)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
      ...
      <div onClick={e => e.stopPropagation()} style={{ background:CARD, borderRadius:16, padding:24, maxWidth:380, width:'100%', boxShadow:'0 12px 32px rgba(0,0,0,0.2)' }}>
        ...
        <form onSubmit={handleSubmit}>
          <label style={{ display:'block', fontSize:14.5, fontWeight:600, color:TEXT, marginBottom:6 }}>Name</label>
          ...
          <label style={{ display:'block', fontSize:14.5, fontWeight:600, color:TEXT, marginBottom:6 }}>Category</label>
          ...
          <p style={{ fontSize:13, color:MUTED, marginTop:-8, marginBottom:14 }}>The stop is added at the end of the category — drag it into position afterwards.</p>

          <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1fr) minmax(0,1fr)', gap:10, marginBottom:14 }}>
            <div>
              <label style={{ display:'block', fontSize:14.5, fontWeight:600, color:TEXT, marginBottom:6 }}>Latitude</label>
            ...
            <div>
              <label style={{ display:'block', fontSize:14.5, fontWeight:600, color:TEXT, marginBottom:6 }}>Longitude</label>
