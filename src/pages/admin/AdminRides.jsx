        ...
        <div style={{ display:'flex', gap:6, background:CARD, border:`1px solid ${BORDER}`, borderRadius:10, padding:4 }}>
          {[[true,'Live'],[false,'All Rides']].map(([val,label]) => (
            <button key={label} onClick={() => setLiveOnly(val)}
              style={{ padding:'7px 16px', borderRadius:8, border:'none', fontSize:14.5, fontWeight:700, cursor:'pointer', fontFamily:'inherit',
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:14 }}>
          ...
          <tbody>
            {loading ? (
              ...
              const s = STATUS_STYLE[r.status] || { bg:CHIP, fg:MUTED, label:r.status }
              return (
                <tr key={r.id} style={{ borderTop:`1px solid ${BORDER}` }}>
                  ...
                  <td style={{ padding:'12px 16px' }}>
                    <span style={{ fontSize:12.5, fontWeight:700, padding:'3px 10px', borderRadius:20, background:s.bg, color:s.fg }}>{s.label}</span>
                  </td>
                  <td style={{ padding:'12px 16px', color:MUTED, fontSize:14.5 }}>{new Date(r.createdAt).toLocaleString()}</td>
                </tr>
              )
            })}
          </tbody>
