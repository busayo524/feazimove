...
export default function AdminBackOffice() {
  ...
  return (
    <AdminLayout title="Back Office">
      ...
      <p style={{ fontSize: 13.5, color: overview?.configured ? '#15803d' : '#d97706', fontWeight: 600, marginBottom: 18 }}>
      ...
      {overview?.rails && (
        <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap', padding:'10px 14px',
          ...
          <span style={{ padding:'3px 10px', borderRadius:50, fontSize:12.5, fontWeight:800, letterSpacing:'0.04em',
        ...
        {stats.map(s => (
          <div key={s.label} style={{ background: CARD, border: `1px solid ${s.alert ? '#fca5a5' : BORDER}`, borderRadius: 14, padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: s.alert ? '#dc2626' : MUTED, marginBottom: 6 }}>
              {s.icon}<span style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{s.label}</span>
            </div>
            <p style={{ fontSize: 22, fontWeight: 800, color: s.alert ? '#dc2626' : TEXT }}>{s.value}</p>
            {s.sub && <p style={{ fontSize: 13, color: MUTED }}>{s.sub}</p>}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
        {[['events', 'Transaction Monitor'], ['customers', 'Customers'], ['aml', 'AML Flags']].map(([key, label]) => (
          <button key={key} onClick={() => setTab(key)}
            style={{ padding: '9px 18px', borderRadius: 50, fontSize: 14.5, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer',
              ...
              <thead>
                <tr style={{ background: BG, textAlign: 'left' }}>
                  {['Time', 'Event', 'Resource', 'Origin', 'Processed'].map(h => (
                    <th key={h} style={{ padding: '10px 14px', fontSize: 13, color: MUTED, textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {!events ? (
                  ...
                  <tr key={e.id} style={{ borderTop: `1px solid #f5f5f5` }}>
                    ...
                    <td style={{ padding: '10px 14px', fontWeight: 700, color: TEXT }}>
                      {e.type}
                      {e.detail && <p style={{ fontWeight: 400, fontSize: 13, color: MUTED, marginTop: 2 }}>{e.detail}</p>}
                    </td>
                    <td style={{ padding: '10px 14px', color: MUTED, fontFamily: 'monospace', fontSize: 13.5 }}>{e.resourceId || '—'}</td>
              ...
              <tbody>
                {!customers ? (
                  ...
                  <tr key={c.id} style={{ borderTop: `1px solid #f5f5f5` }}>
                    ...
                    <td style={{ padding: '10px 14px', fontFamily: 'monospace', fontSize: 13.5, color: MUTED }}>{c.anchorCustomerId}</td>
                      ...
                      {c.payoutBeneficiary ? (
                        <>
                          ...
                          {c.payoutBeneficiary.registered && (
                            <p style={{ fontSize: 12.5, color: '#15803d', fontWeight: 600, marginTop: 2 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            {['open', 'reviewed', 'dismissed', 'all'].map(s => (
              <button key={s} onClick={() => setFlagFilter(s)}
                style={{ padding: '6px 14px', borderRadius: 50, fontSize: 13.5, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer', textTransform: 'capitalize',
            ...
            {!flags ? (
              ...
              <div key={f.id} style={{ background: CARD, border: `1px solid ${BORDER}`, borderLeft: `4px solid ${SEV[f.severity] || MUTED}`, borderRadius: 12, padding: '12px 16px', display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 220 }}>
                  <p style={{ fontWeight: 800, fontSize: 14.5, color: TEXT }}>
                    {f.rule} <span style={{ color: SEV[f.severity], fontSize: 12.5, textTransform: 'uppercase' }}>· {f.severity}</span>
                  </p>
                  <p style={{ fontSize: 14.5, color: TEXT, margin: '3px 0' }}>{f.detail}</p>
                ...
                {f.status === 'open' && (
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => reviewFlag(f.id, 'reviewed')}
                      style={{ padding: '8px 14px', borderRadius: 8, background:ACCENT_FILL, color:ON_ACCENT_FILL, border: 'none', fontWeight: 700, fontSize: 13.5, cursor: 'pointer', fontFamily: 'inherit' }}>
                      Mark Reviewed
                    </button>
                    <button onClick={() => reviewFlag(f.id, 'dismissed')}
                      style={{ padding: '8px 14px', borderRadius: 8, background: 'none', color: MUTED, border: `1.5px solid ${BORDER}`, fontWeight: 700, fontSize: 13.5, cursor: 'pointer', fontFamily: 'inherit' }}>
                      Dismiss
                    </button>
                  </div>
                )}
