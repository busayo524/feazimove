  ...
  return (
    <AdminLayout title={rider.name}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:12, marginBottom:20, flexWrap:'wrap' }}>
        <div>
          <p style={{ color:MUTED, fontSize:16 }}>Rider ID: {rider.id}</p>
          <span style={{ fontSize:15, fontWeight:700, padding:'3px 10px', borderRadius:20, marginTop:6, display:'inline-block',
        ...
        <button onClick={() => navigate('/admin/riders')}
          style={{ display:'flex', alignItems:'center', gap:8, padding:'9px 18px', borderRadius:10,
            background:CARD, border:`1px solid ${BORDER}`, color:TEXT, fontWeight:700, fontSize:16,
      ...
      {/* Stats */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(min(160px,100%),1fr))', gap:14, marginBottom:20 }}>
        {[['Total Trips', rider.rides.length], ['Wallet Balance', `₦${rider.walletBalance.toLocaleString()}`]].map(([l,v]) => (
          <div key={l} style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, padding:16 }}>
            <p style={{ fontSize:15.5, color:MUTED, fontWeight:600, marginBottom:6 }}>{l}</p>
      ...
      <div style={{ background:CARD, border:`1px solid ${active ? '#dff0a8' : BORDER}`, borderRadius:14, padding:18, marginBottom:16 }}>
        <p style={{ fontWeight:800, fontSize:16, color:TEXT, marginBottom:12, display:'flex', alignItems:'center', gap:8 }}>
          <Navigation size={15} color="#3f6212"/> Active Trip
        </p>
        {!active ? (
          <p style={{ color:MUTED, fontSize:16, margin:0 }}>No active trip right now.</p>
        ) : (
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:14, flexWrap:'wrap' }}>
            <div style={{ minWidth:0 }}>
              <p style={{ fontSize:15.5, fontWeight:700, color:TEXT, margin:0 }}>{active.pickup} → {active.destination}</p>
              <p style={{ fontSize:15.5, color:MUTED, margin:'4px 0 0' }}>
            ...
            <span style={{ fontSize:15, fontWeight:700, padding:'4px 12px', borderRadius:50, flexShrink:0,
      ...
      <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1fr) minmax(0,1fr)', gap:16, marginBottom:16 }} className="admin-detail-grid">
        ...
        <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, padding:18, alignSelf:'start' }}>
          <p style={{ fontWeight:800, fontSize:16, color:TEXT, marginBottom:6, display:'flex', alignItems:'center', gap:8 }}>
            <Wallet size={15} color="#3f6212"/> Wallet History
          </p>
          {!rider.walletTransactions?.length ? (
            <p style={{ color:MUTED, fontSize:16, margin:'8px 0 0' }}>No wallet transactions yet.</p>
            ...
            <div key={t.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, padding:'11px 0',
              ...
              <div style={{ minWidth:0 }}>
                <p style={{ fontSize:16, fontWeight:600, color:TEXT, margin:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{t.description}</p>
                <p style={{ fontSize:15.5, color:MUTED, margin:'2px 0 0' }}>{new Date(t.date).toLocaleString()}</p>
              </div>
              <span style={{ fontSize:16, fontWeight:800, flexShrink:0, color: t.type === 'credit' ? '#3f6212' : '#b42318' }}>
        ...
        <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, padding:18, alignSelf:'start' }}>
          ...
          {!rider.ratings?.length ? (
            <p style={{ color:MUTED, fontSize:16, margin:'8px 0 0' }}>No ratings received yet.</p>
            ...
            <div key={r.id} style={{ padding:'11px 0', borderTop: i > 0 ? `1px solid ${BORDER}` : 'none' }}>
              ...
              <p style={{ fontSize:15.5, color:TEXT, margin:'4px 0 0' }}>
      ...
      <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, overflow:'hidden' }}>
        <p style={{ fontWeight:800, fontSize:16, color:TEXT, padding:'16px 18px 0' }}>Recent Trips</p>
        {rider.rides.length === 0 ? (
          <p style={{ color:MUTED, fontSize:16, padding:18 }}>No trips yet.</p>
        ) : rider.rides.map((r, i) => (
          <div key={r.id} style={{ display:'flex', justifyContent:'space-between', padding:'12px 18px',
            ...
            <div>
              ...
              <p style={{ fontSize:15.5, color:MUTED }}>{new Date(r.date).toLocaleString()}</p>
            </div>
            <div style={{ textAlign:'right' }}>
              <p style={{ fontSize:16, fontWeight:700, color:TEXT }}>₦{r.fare.toLocaleString()}</p>
              <p style={{ fontSize:15, color:MUTED, textTransform:'capitalize' }}>{r.status.replace('_',' ')}</p>
            </div>
          </div>
        ))}
      </div>
